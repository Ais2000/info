			...
			<form action="" id="manage_employee">
				...
				<div class="row">
					<div class="col-md-6 border-right">
						...
						<div class="form-group">
							<label for="" class="control-label">Middle Initial (M.I)</label>
						...
						<!-- ADDED MORE INPUTS FOR EMPLOYEE INFO;
							 MUST ADD IT ON THE DATABASE-->
						<div class="form-group">
						<label for="" class="control-label">Gender</label><br>
						<select name="gender" id="gender" class="form-control form-control-sm select2" required value="<?php echo isset($gender) ? $gender : '' ?>">
							<option value="">-- Select gender --</option>
							<option value="female">Female</option>
							<option value="male">Male</option>
						</select>
						<div class="form-group"></div>
						<label for="" class="control-label">Emergency Contact</label><br>
						...
						<input type="text" class="form-control form-control-sm" name="address" required value="<?php echo isset($address) ? $address : '' ?>">
						<div class="error phoneerror"></div>
						</div>
						<div class="form-group">
							<label for="" class="control-label">School</label>
							<select name="school_id" id="school_id" class="form-control form-control-sm select2">
								<option value=""></option>
								<?php 
								$schools = $conn->query("SELECT * FROM school_list order by firstname asc");
								while($row=$schools->fetch_assoc()):
								?>
								<option value="<?php echo $row['id'] ?>" <?php echo isset($school_id) && $school_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['firstname'] ?></option>
								<?php endwhile; ?>
							</select>
						</div>
						<div class="form-group">
					...
					<div class="col-md-6">
						...
						<div class="form-group d-flex justify-content-center align-items-center">
							<img src="<?php echo isset($avatar) ? 'assets/uploads/'.$avatar :'' ?>" alt="Avatar" id="cimg" class="img-fluid">
						</div>
						<div class="form-group">
