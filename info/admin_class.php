...
Class Action {
	...
	function login(){
		extract($_POST);
		$type = array("employee_list","evaluator_list","users","school_list");
	...
	function save_list_school(){
		...
		$chk = $this->db->query("SELECT * FROM list_school where nameofschool = '$nameofschool' and id != '{$id}' ")->num_rows;
		if($chk > 0){
			return 2;
		}
		if(isset($user_ids)){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO list_school set $data");
		}else{
			$save = $this->db->query("UPDATE list_school set $data where id = $id");
		}
		if($save){
			return 1;
		}
	}
	function delete_list_school(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM list_school where id = $id");
		if($delete){
			return 1;
		}
	}
	...
	function delete_employee(){
		extract($_POST);
		$delete = $this->db->query("UPDATE employee_list SET status='inactive' where id = ".$id);
		if($delete)
			return 1;
	}
    ...
    //-------------------------------------------------------------------------------------------------------//
		...
		$check = $this->db->query("SELECT * FROM school_list where email ='$email' ".(!empty($id) ? " and id != {$id} " : ''))->num_rows;
		if($check > 0){
			return 2;
			exit;
		}
		...
		if(!isset($_FILES['img']) || (isset($_FILES['img']) && $_FILES['img']['tmp_name'] == '')){	
			...
			$save = $this->db->query("INSERT INTO school_list set $data");
		}else{
			$save = $this->db->query("UPDATE school_list set $data where id = $id");
		}
	...
	function delete_school(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM school_list where id = ".$id);
		if($delete)
			return 1;
	}
